...
- **Option injection blocked**: pool, dataset, snapshot, device and SSH host regexes now anchor to `^[a-zA-Z0-9]`. Names starting with `-` or `.` are rejected, since they were interpreted as flags of `zpool`/`zfs`/`ssh`. Not exploitable as shell injection, but the whitelist was incorrect.
